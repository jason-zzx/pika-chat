...
### 方案 1：生产环境 Docker Compose 部署（推荐）

通过 Docker 一键拉起 PostgreSQL 数据库和 Pika Chat 应用服务，内置数据卷持久化并自动执行数据库版本迁移。
...
2. **生成加密密钥**：
   ```bash
   # 生成用于 API Key 加密的认证密钥（AES-256-GCM，至少 32 字符）
...
3. **启动生产容器**：
   ```bash
   export CREDENTIAL_ENCRYPTION_SECRET="<替换为你生成的加密密钥>"
   export BETTER_AUTH_SECRET="<替换为你生成的认证密钥>"
   export POSTGRES_PASSWORD="<自定义数据库密码>"
...
### 方案 2：生产环境叠加 RustFS（全内网一体化 S3 存储）

如需在单机中内嵌轻量级高性能 S3 兼容存储且不依赖外部云厂商服务，只需叠加配置运行：

```bash
export CREDENTIAL_ENCRYPTION_SECRET="<替换为你生成的加密密钥>"
export BETTER_AUTH_SECRET="<替换为你生成的认证密钥>"
export S3_ACCESS_KEY_ID="<自定义S3访问密钥>"
export S3_SECRET_ACCESS_KEY="<自定义S3密钥Secret>"
export S3_BUCKET="pika-attachments" # 可选，默认为 pika-attachments
...
- **Node.js**: >= 22
- **pnpm**: 11.9+
- **Docker** 或 **Podman**（用于拉起本地 PostgreSQL 数据库）
...
| 变量名称 | 是否必填 | 默认值 | 说明 |
|---|:---:|:---:|---|
| `DATABASE_URL` | **是** | — | PostgreSQL 连接字符串（例如：`postgres://pika:pika@localhost:5432/pika_chat`）。 |
