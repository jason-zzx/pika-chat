...
export async function uploadFile(
  ...
  // skipSizeLimit is for server-generated bytes (image generation: a 4K PNG
  // outgrows the upload cap): the per-file limit guards user files, while the
  // storage quota below still applies to every byte.
  if (!options?.skipSizeLimit && sizeBytes > limitBytes) {
